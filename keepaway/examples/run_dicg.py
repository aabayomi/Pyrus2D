"""
DICG and DCG Implementation Script. 
Based on the implementation of the following paper:

DICG https://github.com/parachutel/CoordinationGraphMARL
DCG  https://github.com/wendelinboehmer/dcg

This implementation were tested on the Keepaway environment.
...
from keepaway.dicg.torch.policies.dicg_ce_categorical_lstm_policy import (
    DICGCECategoricalLSTMPolicy,
)
from keepaway.dicg.sampler import CentralizedMAOnPolicyVectorizedSampler
from keepaway.dicg.torch.policies.dicg_ce_categorical_mlp_policy2 import (
    DICGCECategoricalMLPPolicy2,
)

results_path = os.path.join(dirname(dirname(abspath(__file__))), "save")
...
def train(args):
    ...
    tabular_log_file = os.path.join(
        args.save_dir, "progress" + continuing_flag + ".csv"
    )
    ...
    vf = CentValueFunction(state_size=env.observation_space.shape[0], device=device)
    ...
    if args.load_path or args.continuing_checkpoint:
        path = (
            args.load_path if args.load_path is not None else args.continuing_checkpoint
        )
