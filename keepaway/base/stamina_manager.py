...
def get_normal_dash_power(wm: "WorldModel", s_recover_mode: bool):
    ...
    if (
        wm.self().unum() <= 5
        and wm.ball().inertia_point(ball_min_reach_cycle).x() < -20.0
    ):
        dash_power = SP.i().max_dash_power()
    elif s_recover_mode:
        ...
        wm.ball().pos().x() > 25.0
        and wm.ball().pos().x() > wm.self().pos().x() + 10.0
        and self_min < opp_min - 6
        and mate_min < opp_min - 6
    ):
        dash_power = smath.bound(
            SP.i().max_dash_power() * 0.1, my_inc * 0.5, SP.i().max_dash_power()
        )
    else:
