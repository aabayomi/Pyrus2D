...
class BhvDribbleGen(BhvKickGen):
    def generator(self, wm: "WorldModel"):
        ...
        if debug_dribble:
            for dribble in self.debug_list:
                if dribble[2]:
                    log.sw_log().dribble().add_message(
                        dribble[1].x(), dribble[1].y(), "{}".format(dribble[0])
                    )
                    log.sw_log().dribble().add_circle(
                        cicle=Circle2D(dribble[1], 0.2), color=Color(string="green")
                    )
                else:
                    log.sw_log().dribble().add_message(
                        dribble[1].x(), dribble[1].y(), "{}".format(dribble[0])
                    ...
                    log.sw_log().dribble().add_circle(
                        cicle=Circle2D(dribble[1], 0.2), color=Color(string="red")
                    )
        end_time = time.time()
        if end_time - start_time > max_dribble_time:
            max_dribble_time = end_time - start_time
        log.sw_log().dribble().add_text(
            "time:{} max is {}".format(end_time - start_time, max_dribble_time)
        )
    ...
    def generate_simple_dribble(self, wm: "WorldModel"):
        ...
        for a in range(angle_div):
            ...
            if wm.self().pos().x() < 16.0 and dash_angle.abs() > 100.0:
                if debug_dribble:
                    log.sw_log().dribble().add_text(
                        "#dash angle:{} cancel is not safe1".format(dash_angle)
                    )
                continue

            if (
                wm.self().pos().x() < -36.0
                and wm.self().pos().abs_y() < 20.0
                and dash_angle.abs() > 45.0
            ):
                if debug_dribble:
                    log.sw_log().dribble().add_text(
                        "#dash angle:{} cancel is not safe2".format(dash_angle)
                    )
                continue
            ...
            if angle_step * a < 180.0:
                ...
                if debug_dribble:
                    log.sw_log().dribble().add_text(
                        "#dash angle:{} turn:{}".format(dash_angle, n_turn)
                    )
    ...
    def simulate_kick_turns_dashes(self, wm: "WorldModel", dash_angle, n_turn):
        ...
        trap_rel = Vector2D.polar2vector(
            ptype.player_size() + ptype.kickable_margin() * 0.2 + 0, dash_angle
        )
        ...
        for n_dash in range(max_dash, min_dash - 1, -1):
            ...
            if ball_trap_pos.abs_x() > max_x or ball_trap_pos.abs_y() > max_y:
                if debug_dribble:
                    log.sw_log().dribble().add_text(
                        "#index:{} target:{} our of field".format(
                            self.index, ball_trap_pos
                        )
                    )
                    self.debug_list.append((self.index, ball_trap_pos, False))
                continue

            term = (1.0 - pow(sp.ball_decay(), 1 + n_turn + n_dash)) / (
                1.0 - sp.ball_decay()
            )
            ...
            if (
                kick_power > sp.max_power()
                or kick_accel.r2() > pow(sp.ball_accel_max(), 2)
                or first_vel.r2() > pow(sp.ball_speed_max(), 2)
            ):
                if debug_dribble:
                    log.sw_log().dribble().add_text(
            ...
            if (wm.ball().pos() + first_vel).dist2(self_cache[0]) < pow(
                ptype.player_size() + sp.ball_size() + 0.1, 2
            ):
                if debug_dribble:
                    log.sw_log().dribble().add_text(
    ...
    def create_self_cache(
        self, wm: "WorldModel", dash_angle, n_turn, n_dash, self_cache
    ):
        sp = SP.i()
        ptype = wm.self().player_type()
        ...
        for i in range(n_dash):
            available_stamina = max(
                0.0, stamina_model.stamina() - sp.recover_dec_thr_value() - 300.0
            )
            dash_power = min(available_stamina, sp.max_dash_power())
            dash_accel = unit_vec.set_length_vector(
                dash_power * ptype.dash_power_rate() * stamina_model.effort()
            )
    ...
    def check_opponent(
        self, wm: "WorldModel", ball_trap_pos: Vector2D, dribble_step: int
    ):
        sp = SP.i()
        ...
        for o in range(12):
            ...
            ball_to_opp_rel = (opp.pos() - wm.ball().pos()).rotated_vector(
                -ball_move_angle
            )
            ...
            if target_dist - control_area < 0.001:
                if debug_dribble:
                    log.sw_log().dribble().add_text(
                        "###OPP {} Catch, ball will be in his body".format(o)
                    )
                return False
            ...
            if n_step - bonus_step <= dribble_step:
                if debug_dribble:
                    log.sw_log().dribble().add_text(
                        "###OPP {} catch n_step:{}, dr_step:{}, bonas:{}".format(
                            o, n_step, dribble_step, bonus_step
                        )
                    )
                return False
            else:
                if debug_dribble:
                    log.sw_log().dribble().add_text(
                        "###OPP {} can't catch n_step:{}, dr_step:{}, bonas:{}".format(
                            o, n_step, dribble_step, bonus_step
                        )
                    )
        return True
