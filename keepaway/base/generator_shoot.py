...
class BhvShhotGen(BhvKickGen):
    def generator(self, wm: "WorldModel") -> ShootAction:
        ...
        if (
            wm.self().pos().x() > SP.i().pitch_half_length() - 1.0
            and wm.self().pos().abs_y() < SP.i().goal_half_width()
        ):
        ...
        for i in range(DIST_DIVS):
            ...
            log.sw_log().shoot().add_text(
                "#shoot {} to {}".format(self.total_count, target_point)
            )
        ...
        self.candidates = sorted(
            self.candidates, key=lambda candidate: candidate.score, reverse=True
        )
        #
        # end_time = time.time()
    ...
    def create_shoot(self, wm: "WorldModel", target_point: Vector2D):
        ...
        if goalie is None or (goalie.unum() > 0 and 5 < goalie.pos_count() < 30):
            ...
            log.sw_log().shoot().add_text(
                "#shoot {} didnt see goalie".format(self.total_count)
            )
            return
        ...
        max_one_step_vel = calc_max_velocity(
            ball_move_angle, wm.self().kick_rate(), wm.ball().vel()
        )
        max_one_step_speed = max_one_step_vel.r()

        first_ball_speed = max(
            (ball_move_dist + 5.0) * (1.0 - sp.ball_decay()), max_one_step_speed, 1.5
        )
        ...
        while not over_max:
            ...
            not_failed = self.check_shoot(
                wm, target_point, first_ball_speed, ball_move_angle, ball_move_dist
            )
            if not_failed:
                ...
                if (
                    self.candidates[-1].goalie_never_reach
    ...
    def check_shoot(
        ...
        ball_reach_step = int(
            math.ceil(
                smath.calc_length_geom_series(
                    first_ball_speed, ball_move_dist, sp.ball_decay()
                )
            )
        )
    ...
    def maybe_goalie_catch(
        self, goalie: "PlayerObject", course: ShootAction, wm: "WorldModel"
    ):
        penalty_area = Rect2D(
            Vector2D(
                SP.i().their_penalty_area_line_x(), -SP.i().penalty_area_half_width()
            ),
        ...
        for cycle in range(min_cycle, max_cycle):
            ball_pos = smath.inertia_n_step_point(
                wm.ball().pos(), course.first_ball_vel, cycle, sp.ball_decay()
            )
            if ball_pos.x() > sp.pitch_half_length():
                break
            in_penalty_area = penalty_area.contains(ball_pos)
            control_area = (
                sp._catchable_area if in_penalty_area else ptype.kickable_area()
            )
            ...
            if goalie.body_count() <= 1:
                Tools.predict_player_turn_cycle(
                    ptype,
                    goalie.body(),
                    goalie_speed,
                    target_dist,
            ...
            bonus_step = (
                smath.bound(0, goalie.pos_count(), 5)
                if in_penalty_area
                else smath.bound(0, goalie.pos_count() - 1, 1)
            )
    ...
    def evaluate_courses(self, wm: "WorldModel"):
        ...
        for it in self.candidates:
            ...
            if it.target_point.dist2(wm.ball().pos()) > y_dist_thr2:
                ...
                y_rate = math.exp(
                    -pow(y_dist, 2.0) / (2.0 * pow(sp.goal_half_width() - 1.5, 2))
                )
