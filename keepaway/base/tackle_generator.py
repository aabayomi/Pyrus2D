...
class TackleGenerator:
    ...
    def generate(self, wm: "WorldModel"):
        ...
        if (
            wm.self().tackle_probability() < 0.001
            and wm.self().foul_probability() < 0.001
        ):
            return
        ...
        if (
            wm.game_mode().time() is not GameModeType.PlayOn
    ...
    def calculate(self, wm: "WorldModel"):
        ...
        tackle_rate = SP.tackle_power_rate() * (1 - 0.5 * ball_rel_angle.abs() / 180.0)
    ...
    def evaluate(self, wm, result: TackleAction):
        SP = ServerParam.i()
        ball_end_point = inertia_final_point(
            wm.ball().pos(), result._ball_vel, SP.ball_decay()
        )
        ...
        if (
            ball_end_point.x() > SP.pitch_half_length()
            ...
            if (
                ...
                and intersect.abs_y() < SP.goal_half_width()
            ):
                shoot_score = 1000000.0

                speed_rate = 1.0 - exp(
                    -(ball_speed**2) / (2 * (SP.ball_speed_max() / 2) ** 2)
                )
        ...
        if ball_end_point.x() < -SP.pitch_half_length():
            ...
            if (
                intersect
                and intersect.is_valid()
                and intersect.abs_y() < SP.goal_half_width() + 1.0
            ):
                y_penalty = -10000.0 * exp(
                    -((intersect.abs_y() - SP.goal_half_width()) ** 2)
                    / (2 * (SP.ball_speed_max() * 0.5) ** 2)
                )
                speed_bonus = 10000 * exp(
                    -(ball_speed**2) / (2 * (SP.ball_speed_max() * 0.5) ** 2)
                )

                shoot_score = y_penalty + speed_bonus
                return shoot_score

        opponent_reach_step = self.predict_opponents_reach_step(
            wm, wm.ball().pos(), result._ball_vel, ball_move_angle
        )
        final_point = inertia_n_step_point(
            wm.ball().pos(), result._ball_vel, opponent_reach_step, SP.ball_decay()
        )

        final_segment = Segment2D(wm.ball().pos(), final_point)
        pitch = Rect2D.from_center(0.0, 0.0, SP.pitch_length(), SP.pitch_width())
        ...
        y_rate = (
            ...
            if final_point.abs_y() > SP.pitch_half_width() - 0.1
                ...
                -(final_point.abs_y() - SP.pitch_half_width())
                / (2 * SP.pitch_half_width() * 0.7) ** 2
            )
        )

        opp_rate = 1 - exp(-(opponent_reach_step**2) / (2 * 30**2))
        score = 10000.0 * our_goal_angle_rate * y_rate * opp_rate
    ...
    def predict_opponents_reach_step(
        ...
        SP = ServerParam.i()
        ball_end_point = inertia_final_point(
            first_ball_pos, first_ball_vel, SP.ball_decay()
        )

        if (
            ball_end_point.abs_x() > SP.pitch_half_length()
            or ball_end_point.abs_y() > SP.pitch_half_width()
        ):
            pitch = Rect2D.from_center(0.0, 0.0, SP.pitch_length(), SP.pitch_width())
            ...
            if len(intersections) == 1:
                first_min_step = SP.ball_move_step(
                    first_ball_vel.r(), first_ball_pos.dist(intersections[0])
                )

        min_step = first_min_step
        for o in wm.their_players():
            step = self.predict_opponent_reach_step(
                o, first_ball_pos, first_ball_vel, ball_move_angle, min_step
            )
    ...
    def predict_opponent_reach_step(
        ...
        for cycle in range(min_cycle, max_cycle):
            ball_pos = inertia_n_step_point(
                first_ball_pos, first_ball_vel, cycle, SP.ball_decay()
            )

            if (
                ball_pos.abs_x() > SP.pitch_half_length()
                or ball_pos.abs_y() > SP.pitch_half_width()
            ):
                return 1000
