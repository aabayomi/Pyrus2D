...
class SampleCoach(CoachAgent):
    ...
    def subsititute_to(self, unum, type):
        if (
            self.world().time().cycle() > 0
            and self.world().our_subsititute_count() >= 3
        ):  # TODO PLAYER PARAM
            log.os_log().error(
                f"(sample coach subsititute to) WARNING: {team_config.TEAM_NAME} coach: over the substitution max."
        ...
        if type not in self.world().available_player_type_id():  # IMP FUNC
            log.os_log().error(
                f"(sample coach subsititute to) type is not available. type={type}"
            )
            return

        self.do_change_player_type(unum, type)
        log.os_log().error(
            f"(sample coach subsititute to) player({unum})'s type changed to {type}"
        )
    ...
    def get_fastest_type(self, candidates: list[PlayerType]):
        ...
        for candidate in candidates:
            ...
            if candidate.cycles_to_reach_max_speed() == min_cycle:
                if (
                    candidate.get_one_step_stamina_consumption()
                    < best_type.get_one_step_stamina_consumption()
                ):
                    best_type = candidate
                    max_speed = best_type.real_speed_max()
