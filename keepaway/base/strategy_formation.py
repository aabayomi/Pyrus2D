...
class _StrategyFormation:
    def __init__(self):
        ...
        if "keepaway.base" in os.listdir("."):
            pwd = "keepaway.base"
        self.before_kick_off_formation: Formation = Formation(
            f"{pwd}/formation_dt/before_kick_off.conf"
        )
        self.defense_formation: Formation = Formation(
            f"{pwd}/formation_dt/defense_formation.conf"
        )
        self.offense_formation: Formation = Formation(
            f"{pwd}/formation_dt/offense_formation.conf"
        )
        self.goalie_kick_opp_formation: Formation = Formation(
            f"{pwd}/formation_dt/goalie_kick_opp_formation.conf"
        )
        self.goalie_kick_our_formation: Formation = Formation(
            f"{pwd}/formation_dt/goalie_kick_our_formation.conf"
        )
        self.kickin_our_formation: Formation = Formation(
            f"{pwd}/formation_dt/kickin_our_formation.conf"
        )
        self.setplay_opp_formation: Formation = Formation(
            f"{pwd}/formation_dt/setplay_opp_formation.conf"
        )
        self.setplay_our_formation: Formation = Formation(
            f"{pwd}/formation_dt/setplay_our_formation.conf"
        )
    ...
    def update(self, wm: "WorldModel"):
        log.os_log().debug(
            f"form{wm.time().cycle()},{wm.time().stopped_cycle()}, {wm.game_mode().type()} {wm.game_mode().is_our_set_play(wm.our_side())}"
        )
        ...
        if (
            self.current_formation is self.before_kick_off_formation
            or wm.game_mode().type()
            in [GameModeType.KickOff_Left, GameModeType.KickOff_Right]
        ):
