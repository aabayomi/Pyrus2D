    ...
    def execute(self, agent: "PlayerAgent"):
        ...
        for unum in range(1, 12):
            ...
            if tm.unum() < 1:
                continue
            for c in range(1, 40):
                dribble_pos = ball_pos + Vector2D.polar2vector(
                    c * dribble_speed_etimate, dribble_angle_estimate
                )
                ...
                tm_cycle = (
                    tm.player_type().cycles_to_reach_distance(
                        tm.inertia_point(opp_min).dist(dribble_pos)
                    )
                    + turn_cycle
                )
        ...
        if blocker == wm.self_unum():
            ...
            log.debug_client().add_message(
                f"block in ({round(block_pos.x(), 2)}, {round(block_pos.y(), 2)})"
            )
