...
class ViewTactical(ViewAction):
    ...
    def execute(self, agent: "PlayerAgent"):
        ...
        if (
            gm is GameModeType.BeforeKickOff
            or gm.is_after_goal()
        ...
        if (
            ViewTactical.PREV1 == 180.0
            and ViewTactical.PREV2 == 180.0
            and agent.world().see_time().cycle() == agent.world().time().cycle() - 2
        ):
            best_width = ViewWidth.WIDE
        elif (
            ViewTactical.PREV1 == 120.0
            and best_width.width() == 60.0
            and agent.world().see_time().cycle() == agent.world().time().cycle() - 1
        ):
    ...
    def get_default_view_width(self, agent: "PlayerAgent"):
        ...
        if wm.self().goalie() and not wm.self().is_kickable():
            ...
            if (
                ball_dist > 10
                or ball_pos.x() > SP.our_penalty_area_line_x()
                or ball_pos.dist(goal_pos) > 20
            ):
                ball_angle = ef.queued_next_angle_from_body(
        ...
        teammate_ball_dist = 1000.0
        opponent_ball_dist = 1000.0
