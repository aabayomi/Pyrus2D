...
from .mlp_encoder_module2 import MLPEncoderModule2
from .attention_module2 import AttentionModule2
from .graph_conv_module2 import GraphConvolutionModule2

class DICGBase2(nn.Module):
    def __init__(self,
                 env_spec,
                 n_agents,
        ...
        self.encoder = MLPEncoderModule2(input_dim=self._dec_obs_dim,
                                        output_dim=self._embedding_dim,
                                        hidden_sizes=encoder_hidden_sizes,
                                        output_nonlinearity=torch.nn.functional.relu)

        self.attention_layer = AttentionModule2(dimensions=self._embedding_dim, 
                                               attention_type=attention_type)

        self.gcn_layers = nn.ModuleList([
            GraphConvolutionModule2(in_features=self._embedding_dim, 
