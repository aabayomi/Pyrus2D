...
class InterceptTable:
    ...
    def update(self, wm: "WorldModel"):
        ...
        if self._last_update_time == wm.time():  # TODO uncomment it
            if InterceptTable.DEBUG:
                log.sw_log().intercept().add_text(
                    "(intercept update) intercept updated before! it called agein"
                )
            return
        ...
        if (
            wm.game_mode().type() == GameModeType.TimeOver
            or wm.game_mode().type() == GameModeType.BeforeKickOff
        ):
            if InterceptTable.DEBUG:
        ...
        if not wm.self().pos().is_valid() or not wm.ball().pos().is_valid():
            log.sw_log().intercept().add_text(
                "(intercept update) self pos or ball pos is not valid"
            )
            return
    ...
    def create_ball_cache(self, wm):
        ...
        for b in self._ball_cache:
            log.sw_log().intercept().add_circle(
                r=0.1, center=b, fill=True, color=Color(string="blue")
            )
    ...
    def predict_self(self, wm):
        ...
        if len(self._self_cache) == 0:
            log.sw_log().intercept().add_text("Intercept self, self cache is empty")
            return
    ...
    def predict_opponent(self, wm: "WorldModel"):
        ...
        for it in opponents:
            ...
            log.sw_log().intercept().add_text(
                f"opp{it.unum()} {it.pos()} " f"type={player_type.id()} cycle={cycle}"
            )
            ...
            if cycle < second_min_cycle:
                ...
                if second_min_cycle < min_cycle:
                    ...
                    self._fastest_opponent, self._second_opponent = (
                        self._second_opponent,
                        self._fastest_opponent,
                    )
    ...
    def predict_teammate(self, wm: "WorldModel"):
        ...
        for it in teammates:
            ...
            cycle = predictor.predict(it, player_type, second_min_cycle)
            log.sw_log().intercept().add_text(
                f"tm{it.unum()} {it.pos()} " f"type={player_type.id()} cycle={cycle}"
            )
            ...
            if it.goalie():
                ...
                if second_min_cycle < min_cycle:
                    # swap :)
                    min_cycle, second_min_cycle = second_min_cycle, min_cycle
                    self._fastest_teammate, self._second_teammate = (
                        self._second_teammate,
                        self._fastest_teammate,
                    )
