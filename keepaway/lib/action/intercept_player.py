...
class PlayerIntercept:
    ...
    def predict(self, player: PlayerObject, player_type: PlayerType, max_cycle: int):
        wm = self._wm
        penalty_x_abs = (
            ServerParam.i().pitch_half_length() - ServerParam.i().penalty_area_length()
        )
        ...
        min_cycle = int(floor(ball_to_player.abs_y() / player_type.real_speed_max()))

        if player.is_tackling():
            min_cycle += max(
                0, ServerParam.i().tackle_cycles() - player.tackle_count() - 2
            )

        min_cycle = max(0, min_cycle - min(player.seen_pos_count(), player.pos_count()))
        ...
        for cycle in range(min_cycle, MAX_LOOP):
            ...
            if control_area + player_type.real_speed_max() * (
                cycle + pos_count
            ) + 0.5 < player_pos.dist(ball_pos):
                # never reach
                continue
            if self.can_reach_after_turn_dash(
                cycle, player, player_type, control_area, ball_pos
            ):
                return cycle
    ...
    def predict_final(self, player: PlayerObject, player_type: PlayerType):
        ...
        n_turn = self.predict_turn_cycle(
            100, player, player_type, control_area, ball_pos
        )
    ...
    def predict_turn_cycle(
        ...
        if target_dist < 5 and angle_diff > 90:  # XXX MAGIC NUMBER XXX :|
            # assume back dash
        ...
        while angle_diff > turn_margin:
            max_turn = player_type.effective_turn(ServerParam.i().max_moment(), speed)
            angle_diff -= max_turn
    ...
    def can_reach_after_turn_dash(
        ...
        return self.can_reach_after_dash(
            n_turn, n_dash, player, player_type, control_area, ball_pos
        )
    ...
    def can_reach_after_dash(
        ...
        player_pos = inertia_n_step_point(
            ppos, pvel, n_turn + max_dash, player_type.player_decay()
        )
        ...
        if player.side != wm.our_side():
            n_dash -= bound(
                0, pos_count - n_turn, min(6, wm.ball().seen_pos_count() + 1)
            )
        else:
            n_dash -= bound(0, pos_count - n_turn, min(1, wm.ball().seen_pos_count()))
