...
class NeckTurnToBall(NeckAction):
    ...
    def execute(self, agent: "PlayerAgent"):
        ...
        view_half = max(0, next_view_width * 0.5 - 10.0)
        ...
        if SP.visible_distance() * 0.7 < ball_dist < 15 and (
            wm.kickable_teammate()
            or wm.kickable_opponent()
            or (opp and opp.dist_from_ball() < opp.player_type().kickable_area() + 0.3)
            or (
                mate
                and mate.dist_from_ball() < mate.player_type().kickable_area() + 0.3
            )
        ):
            view_half = max(0, next_view_width * 0.5 - 20)

        if len(wm.opponents_from_self()) >= 11 and (
            wm.ball().pos().x() > 0
            ...
            if (
                ball_dist > SP.visible_distance() - 0.3
                or wm.ball().seen_pos_count() > 0
            ):
                min_neck_angle = bound(
                    SP.min_neck_angle(),
                    ball_rel_angle_next.degree() - view_half,
                    SP.max_neck_angle(),
                )

                max_neck_angle = bound(
                    SP.min_neck_angle(),
                    ball_rel_angle_next.degree() + view_half,
                    SP.max_neck_angle(),
                )

                best_angle = NeckScanPlayers.get_best_angle(
                    agent, min_neck_angle, max_neck_angle
                )
            ...
            if best_angle != NeckScanPlayers.INVALID_ANGLE:
                ...
                neck_moment = AngleDeg(
                    target_angle - my_body_next.degree() - wm.self().neck().degree()
                )

                agent.do_turn_neck(neck_moment)
                return True

        left_rel_angle = bound(
            SP.min_neck_angle(),
            ball_rel_angle_next.degree() - view_half,
            SP.max_neck_angle(),
        )
        right_rel_angle = bound(
            SP.min_neck_angle(),
            ball_rel_angle_next.degree() + view_half,
            SP.max_neck_angle(),
        )
        ...
        _, left_sum_count, _ = wm.dir_range_count(
            my_body_next + left_rel_angle, next_view_width
        )
        _, right_sum_count, _ = wm.dir_range_count(
            my_body_next + right_rel_angle, next_view_width
        )
