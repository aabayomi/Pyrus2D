...
class Intercept:
    def __init__(
        self, save_recovery: bool = True, face_point: Vector2D = Vector2D.invalid()
    ):
    ...
    def execute(self, agent: "PlayerAgent"):
        ...
        if table.self_reach_cycle() > 100:
            ...
            log.sw_log().intercept().add_circle(
                center=final_point, r=0.5, color=Color(string="red")
            )
            print("GoToPoint in intercept ", final_point)
            GoToPoint(final_point, 2, ServerParam.i().max_dash_power()).execute(agent)
            return True
        ...
        if best_intercept.dash_cycle() == 0:
            ...
            log.sw_log().intercept().add_circle(
                center=face_point, r=0.5, color=Color(string="red")
            )
            print("Turning to Point in intercept ", final_point)
            TurnToPoint(face_point, best_intercept.reach_cycle()).execute(agent)
            return True
        ...
        if self._save_recovery and not wm.self().stamina_model().capacity_is_empty():
            ...
            if (
                wm.self().stamina() - consumed_stamina
                < ServerParam.i().recover_dec_thr_value() + 1
            ):
                log.sw_log().intercept().add_text(f"last if (do turn)")
                agent.do_turn(0)
                return False

        log.sw_log().intercept().add_text(f"do inertia dash (do dash)")
        log.sw_log().intercept().add_circle(
            center=target_point, r=0.5, color=Color(string="red")
        )

        print("Do inertia dash in intercept ", target_point)
        return self.do_inertia_dash(agent, target_point, best_intercept)

    def do_kickable_opponent_check(self, agent: "PlayerAgent") -> bool:
        wm = agent.world()
        if wm.ball().dist_from_self() < 2 and wm.exist_kickable_opponents():
            ...
            if opp is not None:
                ...
                if attack_pos.dist2(goal_pos) > my_next.dist2(goal_pos):
                    ...
                    log.sw_log().intercept().add_circle(
                        center=attack_pos, r=0.5, color=Color(string="red")
                    )
                    GoToPoint(
                        attack_pos,
                        0.1,
    ...
    def get_best_intercept(
        self, wm: "WorldModel", table: InterceptTable
    ) -> InterceptInfo:
        SP = ServerParam.i()
        ...
        for i in range(MAX):
            ...
            if ball_pos.abs_x() > max_pitch_x or ball_pos.abs_y() > max_pitch_y:
                continue
            ...
            if (
                ...
                if (
                    cache[i].turn_cycle() == 0
                    and cache[i].ball_dist()
                    < wm.self().player_type().catchable_area() * 0.5
            ...
            if (
                ...
                and ball_pos.x() < 47
                and (ball_pos.x() > 35 or ball_pos.x() > wm.offside_line_x())
            ):
                attacker = True
            ...
            if attacker:
                ...
                score = (goal_dist / 100) * math.e ** (-(x_diff**2) / (2 * 100))
            ...
            # forward type
            if ball_vel.x() > 0.1 and cycle <= opp_min - 5 and ball_vel.r2() > 0.6**2:
                score = 100**2 - min(100**2, ball_pos.dist2(goal_pos))
        ...
        if noturn_best is not None and forward_best is not None:
            noturn_ball_vel = (
                wm.ball().vel() * SP.ball_decay() ** noturn_best.reach_cycle()
            )
            noturn_ball_speed = noturn_ball_vel.r()
            if noturn_ball_vel.x() > 0.1 and (
        ...
        if (fastest_pos.x() > -33 or fastest_pos.abs_y() > 20) and (
            cache[0].reach_cycle() >= 10 or fastest_vel.r() < 1.2
        ):
            return cache[0]
        ...
        if noturn_best is not None and nearest_best is not None:
            ...
            if noturn_self_pos.dist2(noturn_ball_pos) < nearest_self_pos.dist2(
                nearest_ball_pos
            ):
                return noturn_best
            ...
            if nearest_best.reach_cycle() <= noturn_best.reach_cycle() + 2:
                nearest_ball_vel = (
                    wm.ball().vel() * SP.ball_decay() ** nearest_best.reach_cycle()
                )
                ...
                if (
                    nearest_best.ball_dist()
                    < wm.self().player_type().kickable_area() - 0.4
                ...
                nearest_self_pos = wm.self().inertia_point(nearest_best.reach_cycle())
                if (
                    nearest_ball_speed > 0.7
                    and nearest_self_pos.dist2(nearest_ball_pos)
        ...
        if (
            ...
            for i in range(MAX):
                if (
                    cache[i].reach_cycle() <= cache[0].reach_cycle() + 3
    ...
    def do_wait_turn(
        self, agent: "PlayerAgent", target_point: Vector2D, info: InterceptInfo
    ):
        wm = agent.world()
        ...
        ball_travel = inertia_n_step_distance(
            wm.ball().vel().r(), info.reach_cycle(), ServerParam.i().ball_decay()
        )
        ...
        if info.reach_cycle() == 1 and info.turn_cycle() == 1:
            ...
            log.sw_log().intercept().add_text(f"do wait turn (1) (TurnToPoint)")
            log.sw_log().intercept().add_circle(
                center=face_point, r=0.5, color=Color(string="red")
            )
        ...
        if info.reach_cycle() > 2:
            ...
            if face_point.x() < my_inertia.x():
                face_point = my_inertia + (wm.ball().pos() - my_inertia).rotated_vector(
                    -90
                )
        ...
        if (
            faced_rel.abs_y()
            > wm.self().player_type().kickable_area() - ball_noise - 0.2
        ):
            return False

        log.sw_log().intercept().add_text(f"do wait turn (2)(TurnToPoint)")
        log.sw_log().intercept().add_circle(
            center=face_point, r=0.5, color=Color(string="red")
        )
        TurnToPoint(face_point).execute(agent)
    ...
    def do_inertia_dash(
        ...
        if (
            wm.ball().seen_pos_count() <= 2
            and wm.ball().vel().r() * ServerParam.i().ball_decay() ** info.reach_cycle()
            < ptype.kickable_area() * 1.5
            and info.dash_angle().abs() < 5
            and target_rel.abs_x()
            < (
                ptype.kickable_area()
            ...
            first_speed = calc_first_term_geom_series(
                target_rel.x(), ptype.player_decay(), info.reach_cycle()
            )
            first_speed = min_max(
                -ptype.player_speed_max(), first_speed, ptype.player_speed_max()
            )
        ...
        if info.reach_cycle() >= 4 and (
            target_rel.abs_x() < 0.5 or abs(used_power) < 5
        ):
            my_inertia = wm.self().inertia_point(info.reach_cycle())
            face_point = self._face_point
            if not face_point.is_valid():
            ...
            if (ball_angle - face_angle).abs() > (
                ServerParam.i().max_neck_angle() + normal_half_width - 10
            ):
