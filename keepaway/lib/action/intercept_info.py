...
class InterceptInfo:
    ...
    def __init__(
        ...
        turn_cycle: int = 10000,
        dash_cycle: int = 10000,
        dash_power: float = 100000,
        dash_angle: float = 0,
        self_pos: Vector2D = Vector2D(-10000, 0),
        ball_dist: float = 100000,
        stamina: float = 0,
    ):
        ...
        if (
            self._mode == InterceptInfo.Mode.EXHAUST
            and self._turn_cycle == 10000
            and self._dash_cycle == 10000
            and self._dash_power == 100000.0
            and self._dash_angle == AngleDeg(0.0)
            and self._self_pos == Vector2D(-10000.0, 0.0)
            and self._ball_dist == 10000000.0
            and self._stamina == 0.0
        ):
            self._valid = False
    ...
    def compare(lhs, rhs):
        return (
            True
            if lhs.reach_cycle() < rhs.reach_cycle()
            else (
                lhs.reach_cycle() == rhs.reach_cycle()
                and lhs.turn_cycle() < rhs.turn_cycle()
            )
        )
    ...
    def __lt__(self, other):
        if self.reach_cycle() < other.reach_cycle():
            return True
        return (
            self.reach_cycle() == other.reach_cycle()
            and self.turn_cycle() < other.turn_cycle()
        )
