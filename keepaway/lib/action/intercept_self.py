...
class SelfIntercept:
    ...
    def predict_one_step(self, self_cache):
        ...
        control_area: float = (
            wm.self().player_type().catchable_area()
            if goalie_mode
            else wm.self().player_type().kickable_area()
        )
    ...
    def predict_no_dash(self, self_cache) -> bool:
        ...
        goalie_mode: bool = self.is_goalie_mode(ball_next)
        control_area: float = (
            me.player_type().catchable_area()
            if goalie_mode
            else me.player_type().kickable_area()
        )
        ...
        if next_ball_dist > control_area - 0.15 - ball_noise:
            log.sw_log().intercept().add_text(
                "----->>>> NO ball is out of intercept no dash area"
            )
            return False
    ...
    def is_goalie_mode(self, ball_next, x_limit=None, abs_y_limit=None) -> bool:
        ...
        return (
            ...
            and ball_next.x() < x_limit
            and ball_next.abs_y() < abs_y_limit
        )
    ...
    def predict_one_dash(self, self_cache):
        ...
        goalie_mode: bool = self.is_goalie_mode(ball_next)
        control_area: float = (
            ptype.catchable_area() if goalie_mode else ptype.kickable_area()
        )
        ...
        max_dash_angle = (
            SP.max_dash_angle() + dash_angle_step * 0.5
            if -180 < SP.min_dash_angle() and SP.max_dash_angle() < 180
        ...
        for dash_dir in dirs:
            dash_angle: AngleDeg = me.body() + SP.discretize_dash_angle(
                SP.normalize_dash_angle(dash_dir)
            )
            dash_rate: float = me.dash_rate() * SP.dash_dir_rate(dash_dir)
            log.sw_log().intercept().add_text(
                f"----- dash dir={dash_dir}, angle={dash_angle}, dash_rate={dash_rate}"
            )

            # check recovery save dash
            forward_dash_power = bound(
                0, me.stamina() - SP.recover_dec_thr_value() - 1, SP.max_dash_power()
            )
            ...
            max_back_accel = Vector2D.polar2vector(
                back_dash_power * dash_rate, dash_angle
            )
            ...
            if self.predict_one_dash_adjust(
                dash_angle, max_forward_accel, max_back_accel, control_area, info
            ):
                log.sw_log().intercept().add_text(
                    "---->>>> OK Register 1 dash intercept(1)"
                )
            ...
            if (
                abs(forward_dash_power - SP.max_dash_power()) < 1
                and abs(back_dash_power - SP.min_dash_power()) < 1
            ):
            ...
            if self.predict_one_dash_adjust(
                ...
                log.sw_log().intercept().add_text(
                    "---->>>> OK Register 1 dash intercept(2)"
                )
                tmp_cache.append(info)
                continue
        ...
        for it in tmp_cache:
            if best.ball_dist() > it.ball_dist() or (
                abs(best.ball_dist() - it.ball_dist()) < 0.001
    ...
    def predict_one_dash_adjust(
        ...
        if (
            ball_rel.abs_y() > control_buf
            or Segment2D(forward_accel_rel, back_accel_rel).dist(ball_rel) > control_buf
        ):
            return False
        ...
        if back_accel_rel.x() < ball_rel.x() < forward_accel_rel.x():
            dash_power = self.get_one_step_dash_power(
                ball_rel, dash_angle, forward_accel_rel.x(), back_accel_rel.x()
            )
            log.sw_log().intercept().add_text(
        ...
        if dash_power < -999 and forward_accel_rel.x() < ball_rel.x():
        ...
        # big x difference (<0)
        if dash_power < -999 and ball_rel.x() < back_accel_rel.x():
        ...
        if (
            dash_power < -999
            and back_accel_rel.x() < ball_rel.x() < forward_accel_rel.x()
        ):
        ...
        if (
            stamina_model.stamina() < SP.recover_dec_thr_value()
    ...
    def get_one_step_dash_power(
        ...
        #     return next_ball_rel.x() / dash_rate
        forward_trap_accel_x = (
            next_ball_rel.x()
            - (best_ctrl_dist_forward**2 - next_ball_rel.y() ** 2) ** 0.5
        )
        backward_trap_accel_x = (
            next_ball_rel.x()
        ...
        for accel_x in accels:
            if (0 <= accel_x < max_forward_accel_x) or (max_back_accel_x < accel_x < 0):
    ...
    def predict_short_step(self, max_cycle, save_recovery, self_cache):
        ...
        min_cycle = int(
            ceil(
                (ball_to_self.abs_y() - ptype.kickable_area()) / ptype.real_speed_max()
            )
        )
        ...
        for cycle in range(min_cycle, max_loop + 1):
            ...
            log.sw_log().intercept().add_text(
                f"self pred short cycle {cycle}: bpos={ball_pos}, bvel={ball_vel}"
            )
            ...
            if (control_area + ptype.real_speed_max() * cycle) ** 2 < me.pos().dist2(
                ball_pos
            ):
                log.sw_log().intercept().add_text("self pred short too far")
                continue

            self.predict_turn_dash_short(
                cycle,
                ball_pos,
                control_area,
                save_recovery,  # forward dash
                False,
            ...
            if cycle <= 2:
                self.predict_omni_dash_short(
                    cycle, ball_pos, control_area, save_recovery, False, tmp_cache
                )
            ...
            safety_ball_dist = max(
                control_area - 0.2 - ball.pos().dist(ball_pos) * SP.ball_rand(),
                ptype.player_size() + SP.ball_size() + ptype.kickable_margin() * 0.4,
            )
            best: InterceptInfo = tmp_cache[0]
            for it in tmp_cache[1:]:
                if (
                    best.ball_dist() < safety_ball_dist
                    and it.ball_dist() < safety_ball_dist
                ):
                    if best.turn_cycle() > it.turn_cycle():
                        best = it
                    elif (
                        best.turn_cycle() == it.turn_cycle()
                        and best.stamina() < it.stamina()
                    ):
                        best = it
                else:
    ...
    def predict_omni_dash_short(
        ...
        for dirr in dirs:
            ...
            dash_angle = body_angle + SP.discretize_dash_angle(
            ...
            n_omni_dash, first_dash_power = self.predict_adjust_omni_dash(
                cycle,
                ball_pos,
                control_area,
                save_recovery,
            ...
            for n_dash in range(n_omni_dash + 1, cycle + 1):
                first_speed = calc_first_term_geom_series(
                    (ball_pos - my_pos).rotated_vector(-body_angle).x(),
                    ptype.player_decay(),
                    cycle - n_dash + 1,
                )
                ...
                available_stamina = (
                    max(0, stamina_model.stamina() - recover_dec_thr)
                    if save_recovery
            ...
            if (
                my_pos.dist2(ball_pos) < (control_area - control_area_buf) ** 2
                or my_move.r()
                > (ball_pos - me.pos()).rotated_vector(-my_move.th()).abs_x()
            ):
                mode = (
                    InterceptInfo.Mode.EXHAUST
    ...
    def predict_adjust_omni_dash(
        ...
        dash_angle = body_angle + SP.discretize_dash_angle(
            SP.normalize_dash_angle(dash_rel_dir)
        )
        ...
        for n_omni_dash in range(1, max_omni_dash + 1):
            ...
            dash_power = required_accel / (
                ptype.dash_rate(stamina_model.effort()) * dash_dir_rate
            )
            available_stamina = (
                max(0, stamina_model.stamina() - recover_dec_thr)
                if save_recovery
    ...
    def predict_turn_dash_short(
        ...
        n_turn = self.predict_turn_cycle_short(
            cycle,
            ball_pos,
            control_area,
            back_dash,
    ...
    def predict_dash_cycle_short(
        ...
        for n_dash in range(1, max_dash + 1):
            ...
            first_speed = calc_first_term_geom_series(
                ball_rel.x(), ptype.player_decay(), max_dash - n_dash + 1
            )
            rel_vel = my_vel.rotated_vector(-dash_angle)
            required_accel = first_speed - rel_vel.x()
            dash_power = required_accel / ptype.dash_rate(stamina_model.effort())
        ...
        if my_pos.dist2(ball_pos) < (
            control_area - control_area_buf
        ) ** 2 or me.pos().dist2(my_pos) > me.pos().dist2(ball_pos):
            mode = (
                InterceptInfo.Mode.EXHAUST
    ...
    def predict_turn_cycle_short(
        ...
        if dist_thr < target_dist:
            turn_margin = max(
                self._min_turn_thr, AngleDeg.asin_deg(dist_thr / target_dist)
            )
    ...
    def predict_long_step(self, max_cycle: int, save_recovery: bool, self_cache: list):
        ...
        start_cycle = int(
            ceil(
                (ball_to_self.abs_y() - ptype.kickable_area() - 0.2)
        ...
        for cycle in range(start_cycle, max_loop):
            ...
            if (
                ball_pos.abs_x() > SP.pitch_half_length() + 10
                or ball_pos.abs_y() > SP.pitch_half_width() + 10
            ):
                log.sw_log().intercept().add_text("-------> out of field")
                log.sw_log().intercept().add_circle(
                    cx=ball_pos.x(), cy=ball_pos.y(), r=0.3, color="r"
                )
                break
            ...
            res, n_turn, back_dash, result_recovery = (
                self.can_reach_after_turn_long_dash(
                    cycle, ball_pos, control_area, save_recovery, self_cache
                )
            )
            if res:
                log.sw_log().intercept().add_text(
                    f"-------> {res} turn:{n_turn} back_dash: {back_dash}"
                )
                if not found:
                    max_loop = min(max_cycle, cycle + 10)
                found = True
                log.sw_log().intercept().add_circle(
                    cx=ball_pos.x(), cy=ball_pos.y(), r=0.3, color="green"
                )
            else:
                log.sw_log().intercept().add_text("-------> res not found")
                log.sw_log().intercept().add_circle(
                    cx=ball_pos.x(), cy=ball_pos.y(), r=0.3, color="red"
                )
    ...
    def can_reach_after_turn_long_dash(
        self, cycle, ball_pos, control_area, save_recovery, self_cache
    ) -> tuple:
        dash_angle = self._wm.self().body()
        result_recovery = 0
        n_turn, dash_angle, back_dash = self.predict_turn_cycle(
            cycle, ball_pos, control_area, dash_angle
        )
        if n_turn > cycle:
            return False, n_turn, back_dash, result_recovery

        res, result_recovery = self.can_reach_after_dash(
            n_turn,
            max(0, cycle - n_turn),
            ball_pos,
            control_area,
    ...
    def predict_turn_cycle(
        self, cycle: int, ball_pos: Vector2D, control_area: float, dash_angle: AngleDeg
    ) -> tuple:
        wm = self._wm
        ptype = wm.self().player_type()
        ...
        if n_turn > 0:
            ...
            dash_angle = target_angle + (
                angle_diff if diff_is_positive else -angle_diff
            )

        return n_turn, dash_angle, back_dash

    def can_back_dash_chase(self, cycle: int, target_dist: float, angle_diff: float):
        wm = self._wm
        ...
        if (
            not wm.self().goalie() or wm.last_kicker_side() == wm.our_side()
        ) and cycle >= 5:
            return False

        if wm.self().goalie() and wm.last_kicker_side() != wm.our_side() and cycle >= 5:
            if cycle >= 15:
                return False
        ...
        total_recover = (
            wm.self().player_type().stamina_inc_max()
            * wm.self().recovery()
            * (cycle - 1)
        )
        result_stamina = wm.self().stamina() - total_consume + total_recover
    ...
    def can_reach_after_dash(
        ...
        ball_noise = wm.ball().pos().dist(ball_pos) * SP.ball_rand() * 0.5
        ...
        can_over_speed_max = ptype.can_over_speed_max(
            dash_power_abs, stamina_model.effort()
        )
        ...
        for i in range(n_dash):
            ...
            if (
                available_power < dash_power_abs
                or stamina_model.effort() < prev_effort
                or (not can_over_speed_max and dash_power_abs < available_power)
            ):
            ...
            if can_over_speed_max and tmp_vel.r2() > ptype.player_speed_max2():
                ...
                dash_power_abs = abs(
                    dash_accel_x / ptype.dash_rate(stamina_model.effort())
                )
                tmp_vel.add_x(dash_accel_x)
                can_over_speed_max = ptype.can_over_speed_max(
                    dash_power_abs, stamina_model.effort()
            ...
            stamina_model.simulate_dash(
                ptype, dash_power_abs * (-1 if back_dash else 1)
            )
            ...
            if tmp_pos.x() * PLAYER_NOISE_RATE + 0.1 > noised_ball_x:
                ...
                if my_inertia.dist2(my_final_pos) > 0.01:
                    my_final_pos = Line2D(p1=my_inertia, p2=my_final_pos).projection(
                        ball_pos
                    )
    ...
    def predict_final(self, max_cycle: int, self_cache: list):
        ...
        control_area = (
            ptype.catchable_area() - 0.15 if goalie_mode else ptype.kickable_area()
        )
        dash_angle = me.body()
        n_turn, dash_angle, back_dash = self.predict_turn_cycle(
            100, ball_final_pos, control_area, dash_angle
        )
