...
class GlobalWorldModel:
    def __init__(self):
        ...
        self._our_players: list[GlobalPlayerObject] = [
            GlobalPlayerObject() for _ in range(11)
        ]
        self._their_players: list[GlobalPlayerObject] = [
            GlobalPlayerObject() for _ in range(11)
        ]
        self._unknown_player: list[GlobalPlayerObject] = [
            GlobalPlayerObject() for _ in range(22)
        ]
        ...
        self._subsititute_count: dict[SideID, int] = {SideID.LEFT: 0, SideID.RIGHT: 0}
    ...
    def update_game_mode(self, game_mode: GameMode, current_time: GameTime):
        ...
        if (
            self._game_mode.type() is not GameModeType.PlayOn
            and game_mode.type() is GameModeType.PlayOn
        ):
    ...
    def change_player_type(self, side: SideID, unum: int, type: int):
        if side is SideID.NEUTRAL or not (1 <= unum <= 11):
            log.os_log().error(
                f"(change player type) unum or side is not standard. side={side} unum={unum}"
            )
            return
        ...
        if side == self.our_side() or (
            self.our_side() is SideID.NEUTRAL and side is SideID.LEFT
        ):
