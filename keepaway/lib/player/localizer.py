...
class Localizer:
    ...
    def get_face_dir_by_markers(
        self, markers: list[SeeParser.MarkerT], view_width: ViewWidth
    ):
        ...
        if marker2 is None:
            return None
        marker1_dist_avg, marker1_dist_err = (
            self._object_table.get_landmark_distance_range(view_width, markers[0].dist_)
        )
        marker2_dist_avg, marker2_dist_err = (
            self._object_table.get_landmark_distance_range(
                view_width, markers[-1].dist_
            )
        )
    ...
    def localize_self_simple(self, see: SeeParser, self_face: float):
        ...
        for marker in markers:
            ...
            if Localizer.DEBUG:
                log.sw_log().world().add_text(
                    f"(localize self) considered-marker[{marker.id_}]={marker_pos}"
                )
                log.sw_log().world().add_circle(
                    center=marker_pos, r=0.25, fill=True, color=Color(string="black")
                )
            ...
            if Localizer.DEBUG:
                log.sw_log().world().add_text(
                    f"(localize self) estimated-pos={estimated_pos}"
                )
                log.sw_log().world().add_circle(
                    center=estimated_pos, r=0.25, fill=True, color=Color(string="red")
                )
        ...
        if Localizer.DEBUG:
            ...
            log.sw_log().world().add_circle(
                center=pos, r=0.25, fill=True, color=Color(string="blue")
            )
    ...
    def generate_points(
        ...
        marker_id: MarkerID,
        self_face: float,
        self_face_error: float,
    ):
        marker_pos = self._object_table.landmark_map.get(marker_id)
        if marker_pos is None:
            return
        ave_dist, dist_err = self._object_table.get_landmark_distance_range(
            view_width, marker.dist_
        )
    ...
    def update_points_by_markers(
        self, view_width: ViewWidth, markers, self_face: float, self_face_error: float
    ):
        counter = 0
        for i in range(1, len(markers)):
            if counter >= 30:
                break
            self.update_points_by(
                view_width, markers[i], markers[i].id_, self_face, self_face_error
            )
            self.resample_points(
                view_width, markers[0], markers[0].id_, self_face, self_face_error
            )

    def update_points_by(
        self,
        view_width: ViewWidth,
        marker,
        marker_id,
        ...
        marker_pos = self._object_table.landmark_map.get(marker_id)
        ave_dist, dist_error = self._object_table.get_landmark_distance_range(
            view_width, marker.dist_
        )
    ...
    def resample_points(
        ...
        if len(self._points) == 0:
            self.generate_points(
                view_width, marker, marker_id, self_face, self_face_error
            )
            return
        import random
        ...
        for i in range(len(self._points), 51):
            ...
            self._points.append(
                self._points[choose_index]
                + Vector2D(random.uniform(-0.01, 0.01), random.uniform(-0.01, 0.01))
            )
    ...
    def update_points_by_behind_marker(
        self, view_width, markers, behind_markers, self_pos, self_face, self_face_error
    ):
        ...
        if marker_id == MarkerID.Marker_Unknown:
            return
        self.update_points_by(
            view_width, behind_markers[0], marker_id, self_face, self_face_error
        )
        if len(self._points) == 0:
            return
    ...
    def localize_self(
        ...
        possible_self_pos = [
            self_pos.copy() if isinstance(self_pos, Vector2D) else None
        ]
        if len(behind_markers) == 0:
            return self_pos, self_pos_err, possible_self_pos

        self.update_points_by_behind_marker(
            view_width, markers, behind_markers, self_pos, self_face, self_face_error
        )
        self_pos, self_pos_err = self.average_points()
        possible_self_pos = [
            self_pos.copy() if isinstance(self_pos, Vector2D) else None
    ...
    def localize_ball_relative(
        ...
        ball = see.balls()[0]
        average_dist, dist_error = self._object_table.get_distance_range(
            view_width, ball.dist_
        )
        ...
        if ball.has_vel_:
            ...
            max_x = max(
                max(
                    max(max(rvel1_1.x(), rvel1_2.x()), max(rvel2_1.x(), rvel2_2.x())),
                    max(max(rvel3_1.x(), rvel3_2.x()), max(rvel4_1.x(), rvel4_2.x())),
                ),
                max(
                    max(max(rvel5_1.x(), rvel5_2.x()), max(rvel6_1.x(), rvel6_2.x())),
                    max(max(rvel7_1.x(), rvel7_2.x()), max(rvel8_1.x(), rvel8_2.x())),
                ),
            )
            max_y = max(
                max(
                    max(max(rvel1_1.y(), rvel1_2.y()), max(rvel2_1.y(), rvel2_2.y())),
                    max(max(rvel3_1.y(), rvel3_2.y()), max(rvel4_1.y(), rvel4_2.y())),
                ),
                max(
                    max(max(rvel5_1.y(), rvel5_2.y()), max(rvel6_1.y(), rvel6_2.y())),
            ...
            min_x = min(
                min(
                    min(min(rvel1_1.x(), rvel1_2.x()), min(rvel2_1.x(), rvel2_2.x())),
                    min(min(rvel3_1.x(), rvel3_2.x()), min(rvel4_1.x(), rvel4_2.x())),
                ),
                min(
                    min(min(rvel5_1.x(), rvel5_2.x()), min(rvel6_1.x(), rvel6_2.x())),
                    min(min(rvel7_1.x(), rvel7_2.x()), min(rvel8_1.x(), rvel8_2.x())),
                ),
            )
            min_y = min(
                min(
                    min(min(rvel1_1.y(), rvel1_2.y()), min(rvel2_1.y(), rvel2_2.y())),
                    min(min(rvel3_1.y(), rvel3_2.y()), min(rvel4_1.y(), rvel4_2.y())),
                ),
                min(
                    min(min(rvel5_1.y(), rvel5_2.y()), min(rvel6_1.y(), rvel6_2.y())),
    ...
    def localize_player(
        ...
        average_dist, dist_error = self._object_table.get_distance_range(
            view_width, seen_player.dist_
        )
        average_dir, dir_error = self.get_dir_range(
            seen_player.dir_, self_face, self_face_err
        )
        ...
        if seen_player.has_vel_:
            player.vel_ = Vector2D(
                seen_player.dist_chng_,
        ...
        if (
            seen_player.body_ != SeeParser.DIR_ERR
