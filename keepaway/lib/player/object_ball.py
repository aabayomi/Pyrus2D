...
class BallObject(Object):
    ...
    def inertia_point(self, cycle: int) -> Vector2D:
        return inertia_n_step_point(
            self._pos, self._vel, cycle, ServerParam.i().ball_decay()
        )

    def inertia_final_point(self):
        return inertia_final_point(self.pos(), self.vel(), ServerParam.i().ball_decay())
    ...
    def update_by_last_cycle(self, act: "ActionEffector", game_mode: GameMode):
        ...
        if self.vel_valid():
            ...
            if act.last_body_command() == CommandType.KICK:
                ...
                log.sw_log().world().add_text(
                    f"accel={accel}, ball accel max={SP.ball_accel_max()}"
                )
        ...
        if (
            game_mode.type() == GameModeType.PlayOn
    ...
    def update_by_game_mode(self, game_mode: GameMode):
        ...
        if type.is_corner_kick():
            if self.pos_count() <= 1 and self.rpos().r2() > 3**2:
                self._pos.assign(
                    (SP.pitch_half_length() - SP.corner_kick_margin())
                    * (1 if self.pos().x() > 0 else -1),
                    (SP.pitch_half_width() - SP.corner_kick_margin())
                    * (1 if self.pos().y() > 0 else -1),
                )
            return
    ...
    def update_self_related(self, player: "SelfObject", prev: "BallObject"):
        if self.rpos_count() == 0:
            ...
            if prev.rpos().is_valid() and player.last_move().is_valid():
                self._rpos = (
                    prev.rpos()
                    + self.vel() / ServerParam.i().ball_decay()
                    - player.last_move()
                )
    ...
    def update_by_hear(
        ...
        if BallObject.DEBUG:
            log.sw_log().sensor().add_text(
                f"(update ball by hear) prior_pos={self.pos()} new_pos={heard_pos}"
            )
            log.sw_log().sensor().add_text(
                f"(update ball by hear) prior_vel={self.vel()} new_pos={heard_vel}"
            )
        ...
        if (
            self._ghost_count == 1 and self.pos_count() == 1 and dist_diff < 3
        ) or self.ghost_count() > 1:
            self._pos = heard_pos.copy()
            self._pos_count = 1
        ...
        if self.pos_count() >= 5 or (
            self.pos_count() >= 2
            and (
                dist_diff > sender_to_ball_dist * 0.05 + 1
        ...
        if (
            self.pos_count() > 0
            and sender_to_ball_dist + 1
            < ServerParam.i().visible_distance()
            < self.dist_from_self()
        ):
            self._pos = heard_pos.copy()
            self._pos_count = 1
