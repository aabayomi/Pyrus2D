...
class SelfObject(PlayerObject):
    ...
    def update_angle_by_see(
        self, face: float, angle_face_error, current_time: GameTime
    ):
        self._time = current_time.copy()
    ...
    def update_vel_dir_after_see(self, sense: SenseBodyParser, current_time: GameTime):
        if sense.time() != current_time:
            log.os_log().error(
                "(update vel dir after see) sense time does not match current time"
            )
            return
    ...
    def update_pos_by_see(
        self,
        pos: Vector2D,
        pos_err: Vector2D,
        my_possible_posses: list[Vector2D],
        face: float,
        face_err: float,
        ...
        if self._pos_count == 1:
            ...
            if self._pos_error.x() < pos_err.x():
                new_pos.set_x(
                    pos.x()
                    + (self.pos().x() - pos.x())
                    * (pos_err.x() / (self._pos_error.x() + pos_err.x()))
                )
            ...
            if self._pos_error.y() < pos_err.y():
                new_pos.set_y(
                    pos.y()
                    + (self.pos().y() - pos.y())
                    * (pos_err.y() / (self._pos_error.y() + pos_err.y()))
                )
            ...
            if self._seen_pos_count == 1 and (
                self.has_sensed_collision() or not self.last_move().is_valid()
            ):
    ...
    def update_self_after_sense_body(
        self, sense_body: SenseBodyParser, act: ActionEffector, current_time: GameTime
    ):
        if self._sense_body_time == current_time:
            log.os_log().critical(
                f"(self update after see) called twice at {current_time}"
            )
            return
        ...
        self._kicking = (
            act.last_body_command() == CommandType.KICK
            or act.last_body_command() == CommandType.TACKLE
        )
        self._view_width = sense_body.view_width()

        self._stamina_model.update_by_sense_body(
            sense_body.stamina(),
            sense_body.effort(),
        ...
        if self.face_valid():
            ...
            if (
                not self.has_sensed_collision()
                and self.vel_valid()
                and sense_body.speed_mag()
                ...
                * 0.11
                ...
                if (
                    ...
                    and (
                        estimate_vel.abs_x() < 0.08
                        or estimate_vel.x() * self._vel.x() < 0
                    )
                    and (
                        estimate_vel.abs_y() < 0.08
                        or estimate_vel.y() * self._vel.y() < 0
                    )
                ):
    ...
    def update_ball_info(self, ball: BallObject):
        ...
        if SelfObject.DEBUG:
            log.os_log().debug(
                f"(self obj update ball_info) player_type_id={ptype.id()}"
            )
            log.os_log().debug(
                f"(self obj update ball_info) kickable_area={ptype.kickable_area()}"
            )
        ...
        if ball.dist_from_self() <= ptype.kickable_area():
            ...
            self._kick_rate = ptype.kick_rate(
                ball.dist_from_self(), (ball.angle_from_self() - self.body()).degree()
            )

        if self._last_catch_time.cycle() + SP.catch_ban_cycle() <= self._time.cycle():
            self._catch_probability = ptype.get_catch_probability(
                self.pos(), self.body(), ball.pos(), 0.055, 0.5
            )
        ...
        if tackle_dist > 1e-5:
            tackle_fail_prob = (
                player2ball.abs_x() / tackle_dist
            ) ** SP.tackle_exponent() + (
                player2ball.abs_y() / SP.tackle_width()
            ) ** SP.tackle_exponent()
            foul_fail_prob = (
                player2ball.abs_x() / tackle_dist
            ) ** SP.foul_exponent() + (
    ...
    def update_kickable_state(
        ...
        if (
            ...
            if (
                self_reach_cycle >= 10
                and opponent_reach_cycle
                < min(self_reach_cycle, teammate_reach_cycle) - 7
            ):
                self._kickable = True
                return

            min_cycle = min(
                self_reach_cycle, teammate_reach_cycle, opponent_reach_cycle
            )
            ball_pos = ball.inertia_point(min_cycle)
            if (
                ball_pos.abs_x() > ServerParam.i().pitch_half_length()
                or ball_pos.abs_y() > ServerParam.i().pitch_half_width()
            ):
                self._kickable = True
                return
    ...
    def focus_point(self) -> Vector2D:
        return self._pos + Vector2D.polar2vector(
            self.focus_point_dist(), self.face() + self.focus_point_dir()
        )
