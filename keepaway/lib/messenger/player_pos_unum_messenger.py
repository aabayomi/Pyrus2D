...
class PlayerPosUnumMessenger(Messenger):
    ...
    def encode(self) -> str:
        if not 1 <= self._unum <= 22:
            log.os_log().error(
                f"(player pos unum messenger encode) unum is out of limit. unum={self._unum}"
            )
            return ""
        ...
        if player is None:
            log.os_log().error(
                f"(player pos unum messenger encode) player is None. unum={self._unum}"
            )
            return None
        ...
        pos = player.pos()
        x: float = (
            min_max(-SP.pitch_half_length(), pos.x(), SP.pitch_half_length())
            + SP.pitch_half_length()
        )
        y: float = (
            min_max(-SP.pitch_half_width(), pos.y(), SP.pitch_half_width())
            + SP.pitch_half_width()
        )
    ...
    def decode(
        ...
        log.sw_log().sensor().add_text(
            f"(PlayerPosUnumMessenger decode) receive a player. unum={unum}, pos={pos}"
        )
