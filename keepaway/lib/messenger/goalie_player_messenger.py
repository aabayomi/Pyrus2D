...
class GoaliePlayerMessenger(Messenger):
    CONVERTER = MessengerConverter(
        Messenger.SIZES[Messenger.Types.GOALIE_PLAYER],
        [
            (53.0 - 16.0, 53.0, 160),
            (-20.0, 20.0, 400),
            (0, 360, 360),
    ...
    def encode(self) -> str:
        ...
        if not (1 <= self._player_unum <= 22):
            log.sw_log().communication().add_text(
                f"(goalie player messenger) player unum invalid"
                f": {self._player_unum}"
            )
            return ""
    ...
    def decode(
        ...
        gpx, gpy, gb, pu, ppx, ppy = GoaliePlayerMessenger.CONVERTER.convert_to_values(
            self._message
        )
        ...
        messenger_memory.add_player(
            sender, pu, Vector2D(ppx, ppy), current_time
        )  # TODO IMP FUNC
