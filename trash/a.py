...
# # Define the range for the random integers, for example from 0 to 99
# low, high = 0, 100

# # Create a 4x4 NumPy array of random integers
# array = np.random.randint(low, high, size=(4, 4))

# # Get the index of the maximum value in each row
# argmax_indices = np.argmax(array, axis=1)

# # Convert the result to a list
# argmax_list = argmax_indices.tolist()

# # Print the array and the indices
...
# print(is_staircase_2([1, 2, 3, 4, 5, 6]))


# ``` python

# def create_pyramid(arr) -> list[int]:

#     """
#     helper function to create a pyramid of numbers .

#     Args: list of numbers
#     Returns : Nested list of numbers in the arr
#     """
...
#         # base case to halt recursive call
#         if index >= len(arr):
#             return []
#         # increment the index for each level
#         next_index = index + len(cache) + 1
#         # store select range of index 
#         cache.append(arr[index:next_index])
#         # store the indices in each operation
...
#     """
#     Main function.
#     Args: filename or file path of txt
#     Returns decoded string
#     """
#     # store encoding in a dictionary
#     encoder = {}
#     # read from files line by line and store in dictionary
#     with open(filename, "r") as file:
#         for line in file:
#             key_value = line.strip().split(" ")
#             encoder[int(key_value[0])] = key_value[1]

#     # creates pyramid of number of levels
#     pyramid = create_pyramid(sorted(encoder.keys()))
#     decoded_string = ""
#     # check last item in each level of the pyramid for decoding
#     for item in pyramid:
#         decoded_string = decoded_string + " " + encoder[item[-1]]
#     return decoded_string

# ```

# The function decoder reads from a text file to create a table of encoded strings with keys are integers while strings are values.
# Then creates a pyramid of number with each level of the pyramid increases by one. It returns a decoded string by taking the last item in 
# each levels of the pyramid.

# file = "/Users/abayomi/Desktop/Pyrus2D/coding_qual_input.txt"
# print(decoder(file))
