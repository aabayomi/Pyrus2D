...
"""
    Code modification from the original keep-away.
"""
...
#     # command = [relative('./base/main_keepaway_player.py')] 
...
def launch_monitor(options):
    ...
    command = ['soccerwindow2 &'] + monitor_options
...
def parse_options(args = None, **defaults):
    ...
    parser.add_option(
        ...
        help =
            "Replace last occurrence of # in player file names with the "
...
def run(options):
    ...
    for i in range(2,options.keeper_count+2):
    ...
    # for i in range(options.taker_count):
