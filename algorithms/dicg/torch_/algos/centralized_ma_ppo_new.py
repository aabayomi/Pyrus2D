...
# g_path = os.path.abspath(current_file_path + "/garage")
...
from garage.torch.algos import _Default, filter_valids, make_optimizer, pad_to_last
from garage.torch.utils import flatten_batch

# from dicg.np.algos import MABatchPolopt
from torch_.algos.utils import pad_one_to_last
...
class CentralizedMAPPO(nn.Module):
    ...
    def __init__(
        ...
        self._baseline_optimizer = make_optimizer(
            baseline_optimizer, baseline, lr=baseline_lr, eps=_Default(1e-5)
        )
    ...
    def train_once(self, itr, paths, tbx):
        print("MultiAgentNumTrajs =", len(paths))
        logger.log("Processing samples...")
        obs, avail_actions, actions, rewards, valids, returns, adjs, alive_masks = (
        ...
        for mini_epoch in range(self._optimization_mini_epochs):
            for start in range(0, len(rewards), step_size):
                ...
                alive_masks_ids = (
                    alive_masks_ids.to(self.device) if alive_masks is not None else None
                )
                ...
                if self._clip_grad_norm is not None:
                    ...
                    torch.nn.utils.clip_grad_norm_(
                        self.baseline.parameters(), self._clip_grad_norm
                    )
            ...
            logger.log(
                "Mini epoch: {} | BaselineLoss: {:.5f}".format(
    ...
    def compute_advantages(self, max_path_length, baselines, rewards):
        adv_filter = torch.full(
            (1, 1, 1, max_path_length - 1), self.discount * self._gae_lambda
        ).to(self.device)
    ...
    def _compute_loss(
        ...
        if self.policy.recurrent:
            policy_entropies = self._compute_policy_entropy(
                obs, avail_actions, actions=actions, adjs=adjs, alive_masks=alive_masks
            )
        else:
            policy_entropies = self._compute_policy_entropy(
                obs, avail_actions, adjs=adjs, alive_masks=alive_masks
            )
        ...
        max_path_length = min(
            self.max_path_length, max(valids).item()
        )  # max actual length
        ...
        objective = self._compute_ppo_objective(
            advantages, obs, avail_actions, actions, rewards, adjs, alive_masks
        )
    ...
    def _compute_policy_entropy(
        self, obs, avail_actions, actions=None, adjs=None, alive_masks=None
    ):
    ...
    def _compute_ppo_objective(
        self, advantages, obs, avail_actions, actions, rewards, adjs, alive_masks
    ):
        # Compute constraint
        with torch.no_grad():
            old_ll = self._old_policy.log_likelihood(
                obs, avail_actions, actions, adjs, alive_masks
            )
        new_ll = self.policy.log_likelihood(
            obs, avail_actions, actions, adjs, alive_masks
        )
    ...
    def process_paths(self, paths):
        ...
        obses = torch.stack(
            [
                pad_to_last(path["observations"], total_length=max_path_length, axis=0)
                for path in paths
            ]
        ...
        states = torch.stack(
            [
                pad_to_last(path["states"], total_length=max_path_length, axis=0)
                for path in paths
            ]
        )  # .to(self.device)
        ...
        actions = torch.stack(
            [
                pad_to_last(path["actions"], total_length=max_path_length, axis=0)
                for path in paths
            ]
        )  # .to(self.device)
        ...
        avail_actions = torch.stack(
            [
                pad_one_to_last(
                    path["avail_actions"], total_length=max_path_length, axis=0
                )
                for path in paths
            ]
        ...
        if self.policy.proximity_adj:
            adjs = torch.stack(
                [
                    pad_to_last(path["adjs"], total_length=max_path_length, axis=0)
                    for path in paths
                ]
            ...
            alive_masks = torch.stack(
                [
                    pad_to_last(
                        path["alive_masks"], total_length=max_path_length, axis=0
                    )
                    for path in paths
                ]
