...
def draw_line(image, p1, p2, cell_size=50, line_color='red'):
    gap = cell_size / 2
    start_coords = (p1[1] * cell_size + gap, p1[0] * cell_size + gap)
    end_coords = (p2[1] * cell_size + gap, p2[0] * cell_size + gap)
    line = (start_coords, end_coords)
    ImageDraw.Draw(image).line(line, fill=line_color)
