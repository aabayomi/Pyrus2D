...
env_specs = [
    env_spec for env_spec in envs.registry.all() if "gym.envs" in env_spec.entry_point
]
...
for game_info in [
    ...
    register(
        ...
        kwargs={"grid_shape": grid_shape, "n_agents": n_agents, "n_preys": n_preys},
    )
    ...
    ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ##
    ## PredatorPrey2 copied sheng Li implementation ##

    _game_name2 = "PredatorPrey2{}x{}".format(grid_shape[0], grid_shape[1])

    register(
        id="{}-v0".format(_game_name2),
        entry_point="ma_gym.envs.predator_prey:PredatorPrey2",
        kwargs={"grid_shape": grid_shape, "n_agents": n_agents, "n_preys": n_preys},
    )
    # fully -observable ( each agent sees observation of other agents)
    register(
        id="{}-v1".format(_game_name2),
        entry_point="ma_gym.envs.predator_prey:PredatorPrey2",
        kwargs={
            "grid_shape": grid_shape,
            "n_agents": n_agents,
...
register(
    id="keepaway-v0",
    entry_point="ma_gym.envs.test_env:env",
)
